...
def main():
    ...
    if process:
        if not openai_api_key or not qdrant_url or not qdrant_api_key:
            st.info("Please provide all required API keys and URL to continue.")
            st.stop()
        ...
        vectorestore = get_vectorstore(text_chunks_list, collection_name, qdrant_url, qdrant_api_key)
        
        if vectorestore is None:
            st.error("Failed to create Vector Store. Please check your inputs and try again.")
            st.stop()

        st.write("Vector Store Created...")
        # create qa chain
        num_chunks = 4
        st.session_state.conversation = get_qa_chain(vectorestore, num_chunks, openai_api_key)
...
def get_vectorstore(text_chunks, COLLECTION_NAME, qdrant_url, qdrant_api_key):
    knowledge_base = None  # Initialize variable
    try:
...
def get_qa_chain(vectorstore, num_chunks, openai_api_key):
